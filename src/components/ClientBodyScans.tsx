...
import { Loader2, Plus, TrendingDown, TrendingUp, Scale, Activity } from "lucide-react";
...
const ClientBodyScans = ({ clientId }: Props) => {
  ...
  const { data: scans, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("body_scans" as any).select("*").eq("client_id", clientId).order("scan_date", { ascending: true });
  ...
  const addScan = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("body_scans" as any).insert({
        client_id: clientId,
  ...
  const bmiChart = (scans || []).map((s: any) => ({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add Scan Button / Form */}
      {!showForm ? (
        <Button className="w-full gap-1" onClick={() => setShowForm(true)}>
          <Plus className="w-4 h-4" /> إضافة فحص جديد
        </Button>
      ) : (
        <Card className="p-4">
          <h3 className="font-bold text-card-foreground mb-3">فحص جديد</h3>
          ...
          <div className="grid grid-cols-2 gap-2 mt-3">
            <Button onClick={() => addScan.mutate()} disabled={!height || !weight || !age || addScan.isPending}>
              {addScan.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "حفظ الفحص"}
            </Button>
...
export default ClientBodyScans;
