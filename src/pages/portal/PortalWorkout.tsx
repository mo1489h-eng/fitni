...
import { Check, ArrowLeft, Timer } from "lucide-react";
...
const PortalWorkout = () => {
  ...
  useEffect(() => {
    if (!resting) return;
    if (restTime <= 0) { setResting(false); setRestTime(60); return; }
  ...
  const handleCompleteSet = useCallback(() => {
    ...
    if (isLastSet) { setCurrentExIdx((i) => i + 1); setCurrentSet(1); setActualWeight(""); setActualReps(""); }
    else { setCurrentSet((s) => s + 1); setResting(true); setActualWeight(""); setActualReps(""); }
      ...
      <ClientPortalLayout>
        <div className="flex flex-col items-center justify-center min-h-[70vh] text-center animate-fade-in space-y-6">
          ...
          <div className="flex gap-6 text-center">
            <div><p className="text-2xl font-bold text-primary">{exercises.length}</p><p className="text-xs text-muted-foreground">تمارين</p></div>
        ...
        <div className="flex flex-col items-center justify-center min-h-[70vh] text-center animate-fade-in space-y-6">
          ...
          <div className="relative w-36 h-36">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="45" fill="none" stroke="hsl(142, 76%, 36%)" strokeWidth="4" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashOffset} className="transition-all duration-1000 ease-linear" />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center"><span className="text-4xl font-bold text-foreground">{restTime}</span></div>
          </div>
    ...
    <ClientPortalLayout>
      <div className="space-y-5 animate-fade-in">
        ...
        <Card className="p-5 text-center">
          <span className="text-xs px-2.5 py-1 rounded-full bg-primary/10 text-primary font-medium">{ex.muscle}</span>
          <h1 className="text-2xl font-bold text-card-foreground mt-3 mb-2">{ex.name}</h1>
          <p className="text-muted-foreground">المطلوب: {ex.sets} سيت × {ex.reps} تكرار{ex.weight > 0 && ` × ${ex.weight} كجم`}</p>
        </Card>
